 ...
 * @author graham
 */
public record MemStoreFeatureSource(MapModel<SimpleFeatureType, MemFeature> map,
  SimpleFeatureType featureType, STRtree featureIndex)
  implements IFeatureSource<SimpleFeatureType, MemFeature>
{
  ...
  @Override
  public IFeatureSourceIterator<MemFeature> features(
    ReferencedEnvelope bounds, double displayScale,
    boolean scaleLess)
  {
    Collection<MemFeature> featuresInScale;

    var ebounds = System.currentTimeMillis();
    @SuppressWarnings("unchecked") List<MemFeature> featuresInBounds = featureIndex.query(bounds);
