 ...
 * record will contain the bounds of the shape and will only read the shape when the
 * shape() method is called. This ShapefileReader.Record is the same object every time, so
 * if you need data from the Record, be sure to copy it.
 *
 * @author jamesm
 * @author aaime
...
public class MyShapeFileReader {
	...
	//
	// /**
	// * Used to mark the current shape is not known, either because someone moved the
	// * reader to a specific byte offset manually, or because the .shx could not be
	// opened
	// */
	// private static final int UNKNOWN = Integer.MIN_VALUE;
	//
	// /**
	// * The reader returns only one Record instance in its lifetime. The record contains
	...
	// return "Record " + number + " length " + length + " bounds " + minX + "," + minY +
	// " " + maxX + "," + maxY;
	// }
	//
	...
	// return geometryFactory.createMultiPoint(new Point[] {
	...
	// * @param strict True to make the header parsing throw Exceptions if the version or
	// * magic number are incorrect.
	// * @throws IOException If problems arise.
	// * @throws ShapefileException If for some reason the file contains invalid records.
	// */
	// public MyShapeFileReader(FileChannel shapefileFiles, boolean strict, boolean
	// useMemoryMapped, GeometryFactory gf)
	...
	// * @param onlyRandomAccess When true sets up the reader to do exclusively read
	// driven
	// * by goTo(x) and thus avoids opening the .shx file
	// * @throws IOException If problems arise.
	// * @throws ShapefileException If for some reason the file contains invalid records.
	// */
	// public MyShapeFileReader(FileChannel shapefileFiles, boolean strict, boolean
	// useMemoryMapped, GeometryFactory gf,
	// boolean onlyRandomAccess) throws IOException, ShapefileException {
	...
	// }
	//
	// private void doInit(FileChannel shapefileFiles, boolean strict, boolean
	// useMemoryMapped, GeometryFactory gf,
	// boolean onlyRandomAccess) throws IOException, ShapefileException {
	...
	// private ByteBuffer ensureCapacity(ByteBuffer buffer, int size, boolean
	...
	// public static int fill(ByteBuffer buffer, ReadableByteChannel channel) throws
	// IOException {
	...
	// private void init(boolean strict, GeometryFactory gf) throws IOException,
	// ShapefileException {
	// geometryFactory = gf;
	//
	...
	//
	// /**
	// * If there exists another record. Currently checks the stream for the presence of 8
	// * more bytes, the length of a record. If this is true and the record indicates the
	// * next logical record number, there exists more records.
	...
	// * If there exists another record. Currently checks the stream for the presence of 8
	// * more bytes, the length of a record. If this is true and the record indicates the
	// * next logical record number (if checkRecord == true), there exists more records.
	...
	// private void positionBufferForOffset(ByteBuffer buffer, int offset) throws
	// IOException {
	...
	// }
	//
	// // Check to see if requested offset is already loaded; ensure that record header
	// // is in the
	// // buffer
	...
	// throw new IllegalStateException("ShapeType changed illegally from " + fileShapeType
	// + " to " + recordType);
	// }
	//
	...
	// *
	// * <ul>
	// * <li>it's your responsibility to ensure the offset corresponds to the actual
	// * beginning of a shape struct
	// * <li>once you call this, reading with hasNext/next on sparse shapefiles will be
	// * broken (we don't know anymore at which shape we are)
	// * </ul>
	// */
	...
	//
	// /**
	// * Sets the current location of the byteStream to offset and returns the next
	// record.
	// * Usually used in conjuctions with the shx file or some other index file. Mind
	// that:
	// *
	// * <ul>
	// * <li>it's your responsibility to ensure the offset corresponds to the actual
