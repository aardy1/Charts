...
public class Coordinates {
	...
	public static double distanceAcross(MapViewModel map) throws TransformException, FactoryException {
		return distanceAcross(map.bounds());
	}

	public static ReferencedEnvelope zoom(CoordinateReferenceSystem crs, ReferencedEnvelope maxExtent,
			ReferencedEnvelope envelope, double zoomFactor) {
	...
	public static void positionAt(MapViewModel map, double x, double y)
		...
		var defaultHeight = world.getHeight() <= 2 * DEFAULT_HEIGHT ? world.getHeight() / 8 : DEFAULT_HEIGHT;
	...
	public static ReferencedEnvelope clip(MapViewModel map, ReferencedEnvelope envelope) {
		return clip(map.viewPortBounds(), envelope, map.crs());
	}

	public static ReferencedEnvelope clip(ReferencedEnvelope maxExtent, ReferencedEnvelope envelope,
	...
	public static String envelopeLabel(ReferencedEnvelope bounds) {
		return labelLongitude(bounds.getMinX()) + " to " + labelLongitude(bounds.getMaxX()) + " : "
	 ...
	 */
	static boolean SCALE_UNIT_COMPENSATION = Boolean
			.parseBoolean(System.getProperty("org.geotoools.render.lite.scale.unitCompensation", "true"));
