...
public class StyledLayerDescriptorParser<F> {
    ...
    private final IStylePopertyAndFunctionCompiler<F> parsingContext;

    public StyledLayerDescriptorParser(InputStream input, IStylePopertyAndFunctionCompiler<F> parsingContext) {
        this.input = input;
        ...
        while (!done && reader.hasNext()) {
            ...
            if (nextEvent.isStartElement()) {
                ...
                switch (normalize(startElement)) {
                    case XML.STYLED_LAYER_DESCRIPTOR,
                            XML.USER_STYLE,
                            XML.NAMED_LAYER,
                    ...
                    case XML.FEATURE_TYPE_NAME ->
                            builder.featureType(Utils.parseString(reader.nextEvent()));
