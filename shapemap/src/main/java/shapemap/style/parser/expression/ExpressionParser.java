...
package shapemap.style.parser.expression;
...
import shapemap.renderer.Functions;
...
import static shapemap.style.parser.Utils.normalizeKey;
...
public class ExpressionParser {
		...
		while (!done && reader.hasNext()) {
			...
			if (nextEvent.isStartElement()) {
				...
				switch (normalize(startElement)) {
					...
					case shapemap.style.parser.XML.LITERAL -> {
						var value = Utils.parseLiteral(reader.nextEvent());
						push(stack, (feature, geom) -> value);
					}
					case shapemap.style.parser.XML.PROPERTY_NAME -> {
					...
					case shapemap.style.parser.XML.FUNCTION -> {
						startFrame(stack);
						var fName = functionName(startElement);
						push(stack, (feature, geom) -> fName);
					}
					case shapemap.style.parser.XML.EQ, shapemap.style.parser.XML.NE, shapemap.style.parser.XML.LT, shapemap.style.parser.XML.GT, shapemap.style.parser.XML.IS_LIKE, shapemap.style.parser.XML.COALESCE ->
						startFrame(stack);
					default -> {
			...
			if (nextEvent.isEndElement()) {
				...
				switch (tag) {
					...
					case shapemap.style.parser.XML.LITERAL, 
							shapemap.style.parser.XML.PROPERTY_NAME -> {
						// do nothing
					}
	...
	private static String functionName(StartElement startElement) throws XMLStreamException {
		...
		while (iterator.hasNext()) {
			...
			switch (key) {
				case shapemap.style.parser.XML.ATTR_NAME -> {
